...
sort(dirty_iris$Sepal.Width, decreasing = T) 
# Los dos primeros valores parecen demasiado grandes respecto al resto

sort(dirty_iris$Sepal.Width, decreasing = F) 
# Los valores de -3 y 0 no son posibles

sort(dirty_iris$Sepal.Length, decreasing = T)
# Los dos primeros valores parecen demasiado grandes respecto al resto

sort(dirty_iris$Sepal.Length, decreasing = F)
# El valor de 0 no es posible

sort(dirty_iris$Petal.Width, decreasing = T)
# No es posible tener un valor infinito

sort(dirty_iris$Petal.Width, decreasing = F)
# Nada raro aquí

sort(dirty_iris$Petal.Length, decreasing = T)
# Otra vez dos valores demasiado grandes

sort(dirty_iris$Petal.Length, decreasing = F)
# Otro valor de 0 que no tiene sentido
```
...
dirty_iris <- dirty_iris %>%
  mutate(across(where(is.numeric), ~ifelse(. <= 0 | !is.finite(.), NA, .)))
```
...
casos_completos <- sum(complete.cases(dirty_iris))
casos_totales <- nrow(dirty_iris)
porcentaje_completos <- 100*(casos_completos/casos_totales)
cat("Porcentaje de observaciones completas:", porcentaje_completos, "%")
...
```{r , include = TRUE}
madm <- function(x){
  m <- 1.4826*median(abs(x-median(x, na.rm = T)), na.rm = T)
  return(m)
}

clean_iris_calculos <- clean_iris %>% 
  summarise(across(where(is.numeric), list(
            media = ~mean(., na.rm = T),
            desviacion_tipica = ~sd(., na.rm = T),
            mediana = ~median(., na.rm = T),
            madm = ~madm(.),
            iqr = ~IQR(., na.rm = T))))

dirty_iris_calculos <- dirty_iris %>% 
  summarise(across(where(is.numeric), list(
            media = ~mean(., na.rm = T),
            desviacion_tipica = ~sd(., na.rm = T),
            mediana = ~median(., na.rm = T),
...
petal_length_clean_calculos <- clean_iris_calculos %>% 
  select(starts_with("Petal.Length"))

petal_length_dirty_calculos <- dirty_iris_calculos%>% 
  select(starts_with("Petal.Length"))

comparacion_petal_length <- bind_rows(
  clean = petal_length_clean_calculos,
  dirty = petal_length_dirty_calculos,
  .id = "dataset"
)

print(comparacion_petal_length)
```
...
reglasigma <- function(x){
  ...
  desv_est <- sd(x, na.rm = T)
  sigma3 <- 3 * desv_est
  lim_inf <- media-sigma3
  lim_sup <- media+sigma3
  outliers <- (x < lim_inf) | (x > lim_sup)
...
reglahampel <- function(x){
  ...
  dispersion_madm <- madm(x)
  madm3 <- 3*dispersion_madm
  lim_inf <- mediana-madm3
  lim_sup <- mediana+madm3
  outliers <- (x < lim_inf) | (x > lim_sup)
  outliers[is.na(outliers)] <- F
  return(outliers)
}
...
reglaboxplot <- function(x){
  cuantiles <- quantile(x, probs = c(0.25, 0.75), na.rm = T)
  x_s <- cuantiles[["75%"]]
  x_i <- cuantiles[["25%"]]
  IQR <- x_s-x_i
  lim_inf <- x_i-1.5*IQR
...
reglapercentil <- function(x){
  cuantiles <- quantile(x, probs = c(0.05, 0.95), na.rm = T)
  lim_sup <- cuantiles[["95%"]]
  lim_inf <- cuantiles[["5%"]]
  outliers <- (x < lim_inf) | (x > lim_sup)
  outliers[is.na(outliers)] <- F
  return(outliers)
...
outliers_clean_iris <- clean_iris %>% 
  mutate(across(where(is.numeric), list(
    sigma = reglasigma,
    hampel = reglahampel, 
    boxplot = reglaboxplot,
    percentil = reglapercentil),
    .names = "{.col}_outlier_{.fn}"))

outliers_dirty_iris <- dirty_iris %>% 
  mutate(across(where(is.numeric), list(
    sigma = reglasigma,
    hampel = reglahampel, 
    boxplot = reglaboxplot,
...
print(outliers_clean_iris)
print(outliers_dirty_iris)
```
...
# Para clean_iris
outliers_por_metodo_clean <- outliers_clean_iris %>% 
  select(contains("outlier"))
  
sapply(outliers_por_metodo_clean, sum)

# Para dirty_iris
outliers_por_metodo_dirty <- outliers_dirty_iris %>% 
  select(contains("outlier"))
  
sapply(outliers_por_metodo_dirty, sum)

# Vamos a ordenar solo por Petal.Length, pero para el resto de variables sería igual
outliers_petal_length_clean <- outliers_clean_iris %>%
  filter(Petal.Length_outlier_sigma | 
         Petal.Length_outlier_hampel |
         Petal.Length_outlier_boxplot |
         Petal.Length_outlier_percentil) %>%
  select(Petal.Length, starts_with("Petal.Length_outlier")) %>%             
  arrange(Petal.Length)

outliers_petal_length_dirty <- outliers_dirty_iris %>%
  filter(Petal.Length_outlier_sigma | 
         Petal.Length_outlier_hampel |
         Petal.Length_outlier_boxplot |
         Petal.Length_outlier_percentil) %>%
...
print(outliers_petal_length_clean)
print(outliers_petal_length_dirty)
```
...
outliers_clean_iris %>% 
  select(Petal.Length, starts_with("Petal.Length_outlier")) %>% 
  pivot_longer(cols = -Petal.Length, names_to = "Metodo", values_to = "Outlier") %>% 
  mutate(Metodo = sub("Petal.Length_outlier_", "", Metodo)) %>% 
  ggplot(aes(x = Metodo, y = Petal.Length, color = Outlier)) + geom_point(position = "jitter")
```
...
media_petal_length <- mean(dirty_iris$Petal.Length, na.rm = TRUE)

iris_petal_imputados <- dirty_iris %>% 
  mutate(Petal.Length1 = ifelse(is.na(Petal.Length), media_petal_length, Petal.Length))

print(iris_petal_imputados)
```
...
iris_petal_imputados <- iris_petal_imputados %>% 
  group_by(Species) %>% 
  mutate(media_species = mean(Petal.Length, na.rm = T),
         Petal.Length2 = ifelse(is.na(Petal.Length), media_species, Petal.Length)) %>%
  ungroup()
...
iris_imputado_outliers <- iris_petal_imputados %>% 
  mutate(across(Petal.Length2, list(
    sigma = reglasigma,
    hampel = reglahampel, 
    boxplot = reglaboxplot,
    percentil = reglapercentil),
...
iris_imputado_outliers %>%
  mutate(TipoDato = ifelse(is.na(Petal.Length), "Imputado", "Original")) %>%
  filter(Petal.Length2_outlier_percentil == FALSE) %>%
  ggplot(aes(x = Petal.Length1, y = Petal.Length2, color = TipoDato)) +
  geom_point()
```

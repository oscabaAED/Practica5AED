...
casos_completos <- sum(complete.cases(dirty_iris2))
casos_totales <- nrow(dirty_iris2)
porcentaje_completos <- 100*(casos_completos/casos_totales)

clean_iris <- dirty_iris2 %>%
  na.omit()
...
dirty_iris_calculos <- dirty_iris2 %>% 
  summarise(across(where(is.numeric), list(
            media = ~mean(., na.rm = T),
            desviacion_tipica = ~sd(., na.rm = T),
            mediana = ~median(., na.rm = T),
...
reglapercentil <- function(x){
  cuantiles <- quantile(x, probs = c(0.05, 0.95), na.rm = TRUE)
...
outliers_por_metodo <- outliers_clean_iris %>% 
  select(contains("outlier"))
  
sapply(outliers_por_metodo, sum)


# Vamos a ordenar solo por Petal.Length de momento, pero podría repetirse para el resto de variables
outliers_petal_length <- outliers_clean_iris %>%
  filter(outlier_ == TRUE) %>%
  select(Petal.Length) %>%             
  arrange(Petal.Length)               

print(valores_outliers_A)
```
